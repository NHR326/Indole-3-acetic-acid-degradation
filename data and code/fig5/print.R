                                                         ...
                                                         ifelse(data$environment=="Wastewater",21,
														        ifelse(data$environment=="Plants",24,
														               ifelse(data$environment=="shoot",27,
														                      ifelse(data$environment=="root",30,
														                             ifelse(data$environment=="built environment",33,34)))))))))))
...
geom_vline(xintercept = 33, linetype=2,color = "grey") +
geom_scatterpie(aes(x=environment, y=Order, r=r1), data=data,cols=colnames(data)[3:4],color=NA) + 
 ...
 #geom_scatterpie_legend(data$r, x=3.5, y=-1.3,n=4,labeller = function(x) round(2^(x*25)))+ 
  ...
  scale_x_continuous("",expand = c(0,0),limits = c(1,35),breaks = c(3,6,9,12,15,18,21,24,27,30,33),labels = c("animal gut","human gut","human oral","Aquatic","human skin","Soil","Wastewater","Plants","shoot","root","built environment"))+
